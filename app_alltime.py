...
# Uses DB-API `?` parameter binding
# -----------------------------
def run_query(query: str, params: Optional[Tuple[Any, ...]] = None) -> pd.DataFrame:
...
# -----------------------------
# Cached query wrappers (TTL to limit frequency)
# All queries are filtered by the selected symbol at the SQL level
# -----------------------------
...
@st.cache_data(ttl=60, show_spinner=False)
def q_series_sym_all(symbol: str) -> pd.DataFrame:
    # NOTE: No time filter → fetch the entire history for the symbol
    q = f"""
...
series_df  = q_series_sym_all(sel_symbol)
...
if st.sidebar.button("Refresh now"):
    ...
    q_series_sym_all.clear()
    st.rerun()  # Streamlit 1.32+ API
